...
if __name__ == "__main__":
    ...
    ret_pkt = cache.process_packet(0,test_store)
    ...
    ret_pkt = cache.process_packet(0,test_load)
    ...
    for i in range(256):
        test_store = Packet(load=False, addr=0x0900 + i, size=1, data=i, latency=1)
        ret_pkt = cache.process_packet(0,test_store)

        test_load = Packet(load=True, addr=0x0900 + i, size=1, data=i, latency=1)
        ret_pkt = cache.process_packet(0,test_load)
